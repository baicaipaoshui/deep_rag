...
class MySQLConnector:
    ...
    def fetch_documents(self) -> list[dict[str, Any]]:
        ...
        for idx, row in enumerate(rows, start=1):
            doc_id = str(row.get("id")) if row.get("id") is not None else f"row_{idx}"
            title = str(row.get("title") or f"Document {idx}")
            content = str(row.get("content") or "")
            updated_at = str(row.get("updated_at") or datetime.now(timezone.utc).isoformat())
            source_table = str(row.get("source_table") or self.database)
